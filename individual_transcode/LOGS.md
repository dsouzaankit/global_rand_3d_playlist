...
## Under `F:\f1_media\3d_fullsbs_trans\fisheye_temp\logs\` (global, shared across clips)
...
Preferred root: `F:\f1_media\3d_fullsbs_trans` (Skybox web-client DLNA share path). If `F:` is missing, `Ensure-DlnaSegmentRoot` in `Invoke-LeafFfmpegControl.ps1` keeps that same path working by storing under `%AppData%\3d_playlist_local` and mapping `F:` via `subst` + a directory junction — do not repoint Skybox at AppData.

**Lifecycle:** workflow **start** calls `Ensure-DlnaSegmentRoot -Force` (create tree if needed) and **restores** any `<sha256>.tmp` media (and `fisheye_temp\avs\*.avs`) using scrambled `.dlna_obf_map.json` (also restores legacy `_v.tmp` / `_dlna_obf_*` / plaintext map files if present). Workflow **quit** (`finally` in flat orchestrator / fisheye batch / hybrid batch) calls `Obfuscate-DlnaSegmentRootMedia` — stop leaf `3d_op_*` ffmpeg, rename media **and** `.avs` / `.avsi` in place to `<sha256(relativePath)>.tmp` (no playable extension; writes/updates scrambled `.dlna_obf_map.json`), and purge logs unless `-KeepLogs`. Media are **not** deleted on quit. Then `Remove-DlnaSegmentRootSubst` tears down our AppData `subst F:` + junction if present (success / abort / timeout); a real `F:` volume and `%AppData%\3d_playlist_local` are left alone. On **error** (clip/child failures, fatal batch stop), `-KeepLogs` retains `*.log` / `logs\` (including `fisheye_temp\logs`). **Batch timeout (124) and user cancel (130) purge those DLNA-root logs** like clean success. **Manual delete** (old clear-on-quit): script `Cleanup-DlnaSegmentRoot.ps1` beside `Readme.txt` calls function `Clear-DlnaSegmentRootContents` (covers `flat\` / `fisheye\` / `hybrid\` / `fisheye_temp\` under the shared root). Playlist-local `transcode_logs\` are never under this root and are unaffected.

| Path | Notes |
|------|--------|
| `F:\f1_media\3d_fullsbs_trans\hybrid\3d_op_00_Full_SBS.mkv`, `3d_op_01_Full_SBS.mkv` / `*_LR_180_FISHEYE.mkv` / `*_LR_180.mkv` | Hybrid batch multiplex: flat + fisheye + **already-3D as-is** (`Run-SegmentCopyAsIs` `-c copy -re` → `LR_180`) share one folder; gradual handoff keeps ~two playable. |
| `F:\f1_media\3d_fullsbs_trans\fisheye\3d_op_00_LR_180_FISHEYE.mkv`, `3d_op_01_LR_180_FISHEYE.mkv` | Dedicated fisheye batch / context-menu DLNA double-buffer (~**60 seconds** per segment; trial `LR_180_FISHEYE`). |
| `F:\f1_media\3d_fullsbs_trans\flat\3d_op_00_Full_SBS.mkv`, `3d_op_01_Full_SBS.mkv` | Dedicated flat orchestrator / context-menu DLNA double-buffer (Skybox `Full_SBS`). |
| `F:\f1_media\3d_fullsbs_trans\fisheye_temp\avs\StreamTo3D.fisheye_temp.*.avs` | Fisheye pass-2 chase scripts. Obfuscated on quit with the rest of the DLNA root (same `<sha256>.tmp` map). `Purge-OldAvs.ps1` does **not** touch this folder unless you pass `-AvsFolder`. |
...
**Default:** **delete** all `*.log` under every discoverable playlist `transcode_logs\` tree (`F:\f1_media`, `P:\bbf_media`, `P:\all_scripts`, `F:\all_scripts`) — including **`media_folder_watcher\`**, **`hybrid_batch*\`**, fisheye batch folders, orchestrator/ffmpeg children — and under `fisheye_temp\logs\`. Removes stale `*.finished` markers under `fisheye_batch_prepare\`. Logs held open by a running encode/watcher are truncated to 0 bytes instead of deleted.
