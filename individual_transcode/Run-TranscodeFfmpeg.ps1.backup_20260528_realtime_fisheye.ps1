...
.PARAMETER FisheyeEyeSize
  Square output pixels per eye for -Fisheye (default 3840). Final SBS width is 2x this value.

.PARAMETER FisheyeInputHFov
  v360 input horizontal FOV (degrees) for each flat eye half. Default 90.

.PARAMETER FisheyeInputVFov
  v360 input vertical FOV (degrees) for each flat eye half. Default 56 (typical 16:9-ish half-frame).
...
param(
    ...
    [switch] $NoClampSeek,
    [switch] $Fisheye,
    [int] $FisheyeEyeSize = 3840,
    [double] $FisheyeInputHFov = 90,
    [double] $FisheyeInputVFov = 56,
...
function Get-FisheyeSbsFilterComplex {
    ...
    $v360 = "v360=input=flat:output=fisheye:ih_fov=${ih}:iv_fov=${iv}:h_fov=${oh}:v_fov=${ov}:w=${EyeSize}:h=${EyeSize}:interp=lanczos:alpha_mask=1,format=yuva420p"
    return "color=c=black:s=${EyeSize}x${EyeSize}[bgl];color=c=black:s=${EyeSize}x${EyeSize}[bgr];" +
        ...
        "[lflat]${v360}[fgl];[bgl][fgl]overlay=shortest=1[lout];" +
        "[rflat]${v360}[fgr];[bgr][fgr]overlay=shortest=1[rout];" +
...
try {
    ...
    if ($useFisheyeSbs) {
        $fisheyeFilter = Get-FisheyeSbsFilterComplex -EyeSize $FisheyeEyeSize `
            -InputHFov $FisheyeInputHFov -InputVFov $FisheyeInputVFov `
            -OutputHFov $FisheyeOutputHFov -OutputVFov $FisheyeOutputVFov
        Write-Host "Fisheye SBS: per-eye v360 after AviSynth (${FisheyeEyeSize}x${FisheyeEyeSize} per eye, ih/v=${FisheyeInputHFov}/${FisheyeInputVFov}, out h/v=${FisheyeOutputHFov}/${FisheyeOutputVFov})"
        $argList += @(
            '-i', $fullInput,
